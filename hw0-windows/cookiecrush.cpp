...
int width = 920, height = 730; // i have set my window size to be 980 x 660
...
// DEFINING FUNCTION FOR AUTO CRUSHING
void autocrush()
{
	for (int i = 0; i <= 10; i++)
	{
		for (int j = 0; j < 15; j++)
		{
			if ((randomfun[i][j] == randomfun[i + 1][j]) && (randomfun[i + 1][j] == randomfun[i + 2][j]) || (randomfun[i][j] == randomfun[i][j + 1]) && (randomfun[i][j + 1] == randomfun[i][j + 2]))
			{
				// Taking a counter for number of consecutive cookies
				int count = 0;
				for (int i2 = i; randomfun[i2][j] == randomfun[i2 + 1][j]; i2++)
				{
					count += 1;
				}
				int count1 = 0;

				for (int j2 = j; randomfun[i][j2] == randomfun[i][j2 + 1]; j2++)
				{
					count1 += 1;
				}
				int i2 = i;
...
// --------------------- DEFINING FUNCTION FOR FILLING THE CANVAS -------------------------
void fill()
{
	int temp;
	for (int i = 0; i <= 10; i++)
	{
		for (int j = 0; j < 15; j++)
		{
			if (randomfun[i][j] == 50)
			{
				//cout << " I coordinate is " << i << " j is " << j<<endl;
				int index = i;
				while (index < 11)
				{
					temp = randomfun[index][j];
					randomfun[index][j] = randomfun[index + 1][j];
					randomfun[index + 1][j] = temp;
					index += 1;
				}
				/*for (int i2 = 1; i2<11; i2++)
				{
				temp = randomfun[i2][j];
				randomfun[i2][j] = randomfun[i2 + 1][j];
				randomfun[i2 + 1][j] = temp;
				}*/
			}
		}
	}
...
void randomgen()
{
	...
	for (int i = 0; i <= 10; i++)
	{
		...
		for (int j = 0; j < 15; j++)
		{
			if (randomfun[i][j] == 50)
			{
				randomfun[i][j] = GetRandInRange(0, 6);
				DrawCookie(Cookies(randomfun[i][j]), a, b, cwidth, cheight);
				a += 60;
			}
		}
		b += 60;
...
void DisplayFunction() {
	...
	//#if 0
	glClearColor(0.3/*Red Component*/, 0.6/*Green Component*/,
	...
	fill();
	randomgen();
	// Changing the location of the score and changing it's color.
	DrawString(800, height - 10, width, height + 10, "Score: " + Num2Str(score), colors[GREEN_YELLOW]);
	DrawString(5, height - 10, width, height + 10, "Welcome to Cookie crush :) ", colors[BLUE_VIOLET]);
	DrawString(400, height - 40, width, height + 10, " Timer:  " + Num2Str(lastframe), colors[RED]);
	DrawString(300, height - 10, width, height + 10, " You have 120 seconds, Goodluck :)  ", colors[DARK_BLUE]);
...
*
* */
int iref1, iref2, jref1, jref2, index = 0;
void MouseClicked(int button, int state, int x, int y) {
	...
	if (button == GLUT_LEFT_BUTTON) // dealing only with left button
	{
		if (state == GLUT_DOWN && gamestate == Ready) {
			cout << "Left Mouse Button Pressed at Coordinates X=" << x << " Y= "
				<< height - y << endl;
			// STORING TWO CLICKS IN VARIABLES..
			iref1 = x / 60;
			jref1 = (height - y) / 60;
			cout << "iref1= " << iref1 << " jref1= " << jref1 << endl;
		}

		cout << "iref2= " << iref2 << " jref2= " << jref2 << endl;
	...
	if (state == GLUT_UP && gamestate == Ready) {
		...
		if (iref2 == iref1 + 1 && jref2 == jref1)
		{
			temp = randomfun[jref1][iref1];
			randomfun[jref1][iref1] = randomfun[jref1][iref1 + 1];
			randomfun[jref1][iref1 + 1] = temp;
		}
		if (iref2 == iref1 - 1 && jref2 == jref1)
		{
			temp = randomfun[jref1][iref1];
		...
		if (jref2 == jref1 + 1 && iref2 == iref1)
		{
			temp = randomfun[jref1][iref1];
			randomfun[jref1][iref1] = randomfun[jref1 + 1][iref1];
			randomfun[jref1 + 1][iref1] = temp;
		}
		if (jref2 == jref1 - 1 && iref2 == iref1)
		{
			temp = randomfun[jref1][iref1];
